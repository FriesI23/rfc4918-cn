...
受保护属性 (protected property) 指不能通过 PROPPATCH 请求更改的属性.
可能有其他请求会导致受保护属性发生更改 (例如, 当 LOCK 请求影响 `DAV:lockdiscovery` 的值时).
...
- **名称**: creationdate
- **目的**: 记录资源创建的时间和日期
- **数值**: [date-time](z-typedefs.md#date-time) (在 [RFC3339#5.6] 中关于 ABNF 的定义)
- **保护**: **可能[MAY]**是受保护的.
  对于某些服务器认为: 如果这个时间对用户来说是更有意义的 (而不是使用上传的时间),
  则允许修改 `DAV:creationdate` 以反映文档创建的时间.
...
>
> ```xml
> <creationdate>2023-06-03T14:23:30Z</creationdate>
> ```
...
- **目的**: 为资源提供适合呈现给用户的名称.
- **数值**: 任意文本.
- **保护**: **不应[SHOULD_NOT]**受保护. 需要注意的是, 由于这是一条新的要求,
  实现 [RFC2518] 的服务器可能将此属性设置为受保护的.
- **COPY/MOVE 行为**: 在 COPY 和 MOVE 操作中**应该[SHOULD]**保留该属性值.
- **描述**: 包含适合展示给用户的资源描述. 此属性在资源上定义,
  并且该属性**应该[SHOULD]** 具有独立于检索其 `Request-URI` 相同的值
  ...
  而只是在各个资源上更改元数据. 即使在同一集合中，两个资源也可以具有相同的 `DAV:displayname` 值.
...
- **目的**: 包含 `Content-Language` 标头值 (来自 [RFC2616#14.12]),
  就像在没有 `accept` 标头并执行 GET 请求时返回的一样.
- **数值**: [language-tag](z-typedefs.md#language-tag) (`language-tag` 在 [RFC2616#3.10])
- **保护**: **不应[SHOULD_NOT]**受保护, 因此客户端可以重置该语言.
  需要注意的是, 因为这是一个新的要求, 实现 [RFC2518] 的服务器可能会将其作为受保护属性.
- **COPY/MOVE 行为**: 在 COPY 和 MOVE 操作中, **应该[SHOULD]**保留此属性值.
- **描述**: `DAV:getcontentlanguage` 属性**必须[MUST]**在 (任何 GET 请求响应中且包含
...
>
> ```xml
> <getcontentlanguage>zh-cn</getcontentlanguage>
> ```
...
- **目的**: 包含通过 GET 请求 (不包含 `accept` 标头) 返回的 `Content-Type` 标头值
  (参见[RFC2616#14.17]).
- **数值**: [media-type](z-typedefs.md#media-type) (在[RFC2616#3.7]中定义).
- **保护**: 如果服务器更倾向于自行分配内容类型, 则有可能受保护 (见[第 9.7.1 章][SECTION#9.7.1]中的讨论).
- **COPY/MOVE 行为**: 属性值**应该[SHOULD]**在 COPY 和 MOVE 操作中保留.
- **描述**：此属性**必须[MUST]**在 (GET 请求反悔中包含 `Content-Language` 标头的)
  DAV 兼容资源上定义.
...
- **名称**: getetag
- **用途**: 包含 `ETag` 标头的值 (来自 [RFC2616#14.19]), 就如返回一个没有 `accept` 标头的 GET 一样.
- **数值**: [entity-tag](z-typedefs.md#entity-tag) (在 [RFC2616#3.11] 中定义)
...
- **目的**: 包含 `Last-Modified` 标头的值 (来自 [RFC2616#14.19]), 就如返回一个没有
  `accept` 标头的 GET 那样.
- **数值**: [rfc1123-date](z-typedefs.md#rfc1123-date) (在 [RFC2616#3.3.1] 中定义)
...
- **COPY/MOVE 行为**: 此属性值依赖于目标资源的最后修改日期, 而不是源资源的属性值.
  ...
  并且可以在 MOVE 操作中保留该值, 即使 HTTP `Last-Modified` 值**应该[SHOULD]**已被修改.
...
>
> ```xml
> <getlastmodified>Fri, 02 Jun 1982 14:30:00 GMT</getlastmodified>
> ```
...
- **保护**: **必须[MUST]**受保护. 客户端通过 LOCK 和 UNLOCK (而不是通过 PROPPATCH) 更改锁列表.
...
- **描述**: 返回有关谁拥有锁, 拥有的锁的类型, 超时类型与超时剩余时间, 以及相关锁令牌的列表.
  ...
  此属性**不可[NOT]**通过写锁[第 7 章][SECTION#7]进行锁定.
...
- **描述**: 返回一个列表组合, 其内包含在资源上可以指定的锁请求中可能出现的锁范围和访问类型.
  ...
  此属性**不可[NOT]**被写锁[第 7 章][SECTION#7]锁定.
