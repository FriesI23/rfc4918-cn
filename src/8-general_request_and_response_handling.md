...
请求实体正文（[REC-XML]）或 HTTP 标头中编码方法参数信息. 使用 XML
...
当 XML 用于请求或响应正文时, `Content-Type` 类型**应该** \*(SHOULD) 为
`application/xml`. 实现**必须** (MUST) 在请求和响应正文中接受 `text/xml` 和
...
## 8.4. 请求中必要的正文 (Required Bodies in Requests)
...
需要注意的是，在 PUT 响应中 `ETag` 的含义在本文档或 [RFC2616] 中都没有明确定义 (i.e., `ETag` 是否表示资源与 PUT 请求正文等效;
...
time)). `ETag` 代表资源的正文或内容的状态, 除了 `ETag` 没有类似的方法来判断属性是否已更改.
...
## 8.7. 包含错误响应的正文 (Including Error Response Bodies)
...
可能表示缺少必需的标头或者标头格式不正确等等). 关于该错误正文机制的内容将在[第十六章]()
中进行说明.
